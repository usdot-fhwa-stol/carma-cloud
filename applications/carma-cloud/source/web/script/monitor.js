...
const g_oControlsPromise = $.getJSON('controls.json').promise();
var g_oControls
var g_oSources;
var g_oLanes;
var g_oTypes;
...
async function initialize()
{
	...
	g_oMap.showTileBoundaries = document.URL.includes("tile");
	...
	g_oMap.on('load', async function()
	{
		...
		g_oControls = await g_oControlsPromise;
	});
	
	$.post("api/event/types", {"token": sessionStorage.token}, function(data, status, xhr){g_oTypes = JSON.parse(data)});
	$.post("api/event/lanes", {"token": sessionStorage.token}, function(data, status, xhr){g_oLanes = JSON.parse(data)});
	
	g_oMap.on('click', mapClickHandler);
