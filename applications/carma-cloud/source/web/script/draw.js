...
class DrawingControl
{
  onAdd(map)
  {
    ...
    this._container.innerHTML = [{t:'Edit Path', i:'editpath'}, {t:'Group Controls',i:'groupctrl'}, {t:'Future View',i:'future'}, {t:'Select Layers',i:'layers'}]
...
class DebugControl
{
  onAdd(map)
  {
    ...
    this._container.innerHTML = [{t:'Weather Polygon', i:'drawpoly'}, {t:'Toggle Debug',i:'debug'}]
      .reduce((accum, opts) => accum +
          `<button class="mapboxgl-ctrl-icon" type="button" title="${opts.t}">
             <img src="images/button-control/${opts.i}.png" />
           </button>`, '');
...
async function initialize()
{
	g_oMap = new mapboxgl.Map({"container": "mapid", "style": "mapbox/streets-v11.json", "attributionControl": false,
		"minZoom": 14, "maxZoom": 22, "center": [-77.149, 38.956], "zoom": 17});
	...
	g_oMap.addControl(new DrawingControl(), 'top-right');
...
function geoSuccess(oJsonData, sStatus, oJqXHR)
{
	...
	let nFirst = this.url.indexOf('/', 1);
	let sUrl = this.url.substring(this.url.indexOf('/', nFirst) + 1);
