...
if [[ -z "${CC_USER}" ]]; then
    echo "Carma Cloud user not set, using default value"
    USER="ccadmin"
else
    USER="${CC_USER}"
fi
...
if [[ -z "${CC_PSWD}" ]]; then
    echo "Carma Cloud password not set, using default value"
    PASSWORD="admin_testpw"
else
    PASSWORD="${CC_PSWD}"
fi

echo "Add new users with:"
echo "java -cp /opt/tomcat/webapps/carmacloud/ROOT/WEB-INF/classes/:/opt/tomcat/lib/servlet-api.jar cc.ws.UserMgr <username> <password> >> /opt/tomcat/webapps/carmacloud/user.csv"
