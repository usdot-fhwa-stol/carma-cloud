...
public class IHP extends HttpServlet
{
	...
	private final ArrayList<BoundsList> m_oBounds = new ArrayList();
		...
		throws ServletException
	{
		...
		String sBoundsFile = oConfig.getInitParameter("boundsfile");
		readBounds(sBoundsFile);
		...
		boolean bSkipFirst = oCal.get(Calendar.MINUTE) % nPeriod == nPeriod - 1;
		...
		oCal.add(Calendar.MINUTE, bSkipFirst ? 1 + nPeriod : 1);
		...
		oCal.set(Calendar.HOUR_OF_DAY, 6); // 6 utc shouldn't have much traffic in usa
		oCal.add(Calendar.DAY_OF_YEAR, 1);
		m_oTimer.scheduleAtFixedRate(new TimerTask(){@Override public void run(){readBounds(sBoundsFile);}}, oCal.getTime(), 86400000);
	}
	
	
	public void readBounds(String sFile)
	{
		synchronized (m_oBounds)
		{
			Path oPath = Paths.get(sFile);
			if (!Files.exists(oPath))
				return;
			for (BoundsList oList : m_oBounds)
			{
				for (Bounds oBound : oList)
				{
					oBound.cancelCtrls();
				}
			}
